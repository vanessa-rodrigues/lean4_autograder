...
private_repo = git.get_repo(config['private_repo'])
user = git.get_user()
...
try:
    ...
    new_repo = user.create_repo(config['public_repo'].split('/')[1])
    ...
    new_repo = git.get_repo(config['public_repo'])

# gets assignment file
content = private_repo.get_contents(config['assignment_path'])
...
new_repo.create_file(config['assignment_path'], commit_message, assignment_file)
